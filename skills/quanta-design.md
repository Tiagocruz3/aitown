...
Use this for UI, layout, styling, fonts, buttons, components, responsive
composition, empty/loading/error states, and premium polish.

For landing pages, homepages, launch pages, waitlists, pricing pages, or
conversion-focused marketing sections, also read `skills/landing-page.md`.

For SaaS apps, dashboards, CRMs, admin panels, workspaces, editors, tables, and
operational tools, also read `skills/saas-application.md`.

For small focused apps such as calculators, notes, todos, trackers, timers,
converters, and simple tools, also read `skills/small-app-design.md`.
...
compose Quanta into generated apps.
...
Generated apps must look designed, not like raw low-level layouts.
...
| Use | Utility |
|---|---|
| Landing hero | `text-q-display-lg-bold` or `text-q-display-md-bold` |
| App page title | `text-q-headline-sm-semi-bold` or `text-q-headline-md-semi-bold` |
...
- Landing/hero CTA: `marketingPrimary`, `marketingSecondary`,
  `marketingTertiary`, or `marketingGhost`.
- Hero CTAs can use `size="lg"` or `size="xl"`.
...
Every generated app should have credible states:
...
3. Check `@theme { --spacing: 0.25rem; }` is still present after the Quanta
   import to restore normal Tailwind layout spacing in generated apps.
