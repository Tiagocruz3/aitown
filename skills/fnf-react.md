...
profile/credits, also read `skills/auth.md`. For the visible UI around these
hooks, also read `app/packages/quanta/ai/AGENTS.md` and `skills/quanta-design.md`.
...
- It must not add product copy, toasts, auth redirects, plan gates, or visual
  components.
- It pairs with Quanta in generated apps: fnf-react owns data/lifecycles,
...
Use `QueryClientProvider` above the app, then `FnfProvider` where SDK clients
are needed.

For real SDK-backed generation/media/profile/credits, render `FnfProvider` only
inside an authenticated app boundary or pass clients that call authenticated
...
- Use `scopeKey` for user/workspace-aware apps, usually
  `${userId}:${workspaceId}`.
...
If the app uses SDK generation and lacks a feed/history view, treat it as
incomplete.
...
- The job client passed to React helpers must ultimately call a server-side SDK
  ...
  generated app built a GET route/function for a write operation.
...
For generated apps, browser auth display should use `/api/user` from
`skills/auth.md`. Do not use `fnf-react` profile queries as a replacement for
