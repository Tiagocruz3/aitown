...
export function createWorkflowPlatformAdapter(options: WorkflowPlatformAdapterOptions = {}): FnfAdapter {
  ...
  return {
    ...
    listJobs: query => send('GET', jobsListPath(query)),
...
function jobsListPath(query: JobListQuery): string {
  const search = new URLSearchParams()
  append(search, 'type', query.type)
  append(search, 'cursor', query.cursor)
  append(search, 'size', query.size)
  append(search, 'parent_id', query.parentId)
  appendMany(search, 'status', query.status)
  appendMany(search, 'model', query.model)
  const qs = search.toString()
  return qs ? `/jobs/list?${qs}` : '/jobs/list'
}
